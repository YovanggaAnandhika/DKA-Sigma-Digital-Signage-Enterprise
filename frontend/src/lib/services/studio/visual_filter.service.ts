...
export async function getVisualFilters(params?: { search?: string; page?: number; limit?: number }): Promise<{ data: VisualFilter[]; total: number }> {
  const result = await invokeApi<any>('/api/grpc/studio/visual_filter/ListVisualFilters', {
    search: params?.search,
    pagination: { page: params?.page || 1, limit: params?.limit || 25 }
  });
  return {
    data: result.filtersList || result.visualFiltersList || result.itemsList || [],
    total: result.pagination?.totalItems || result.filtersList?.length || result.visualFiltersList?.length || result.itemsList?.length || 0
  };
}

export async function getVisualFilter(id: string): Promise<VisualFilter> {
  const result = await invokeApi<any>('/api/grpc/studio/visual_filter/GetVisualFilter', { id });
...
export async function createVisualFilter(data: Omit<VisualFilter, 'id' | 'createdAt'>): Promise<VisualFilter> {
  const result = await invokeApi<any>('/api/grpc/studio/visual_filter/CreateVisualFilter', data);
  const f = result.filter || result;
...
export async function updateVisualFilter(id: string, data: Partial<Omit<VisualFilter, 'id' | 'createdAt'>>): Promise<VisualFilter> {
  const result = await invokeApi<any>('/api/grpc/studio/visual_filter/UpdateVisualFilter', { id, ...data });
  const f = result.filter || result;
...
export async function deleteVisualFilter(id: string): Promise<boolean> {
  await invokeApi<any>('/api/grpc/studio/visual_filter/DeleteVisualFilter', { id });
  return true;
}
