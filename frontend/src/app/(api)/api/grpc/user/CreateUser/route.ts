...
import { CreateUserRequest } from '@/lib/api/generated/iam/v1/user/user.common_pb';
...
export async function POST(req: NextRequest) {
  try {
    ...
    if(body.email) request.setEmail(body.email); if(body.full_name) request.setFullName(body.full_name); if(body.role_id) request.setRoleId(body.role_id); if(body.password) request.setPassword(body.password);
