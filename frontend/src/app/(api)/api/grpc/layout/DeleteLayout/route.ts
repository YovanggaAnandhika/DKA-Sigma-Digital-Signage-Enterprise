...
import { DeleteLayoutRequest } from '@/lib/api/generated/studio/v1/layout/layout.common_pb';
...
export async function POST(req: NextRequest) {
  try {
    ...
    const request = new DeleteLayoutRequest();
    if(body.id) request.setId(body.id);

    return new Promise((resolve) => {
      client.deleteLayout(request, getGrpcMetadata(token), (error: any, response: any) => {
