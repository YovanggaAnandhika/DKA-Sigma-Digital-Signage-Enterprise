...
import { DeleteVisualFilterRequest } from '@/lib/api/studio/v1/visual_filter/visual_filter.common_pb';
...
export async function POST(req: NextRequest) {
  try {
    ...
    const request = new DeleteVisualFilterRequest();
    if (body.id) request.setId(body.id);
    
    return new Promise<NextResponse>((resolve) => {
      client.deleteVisualFilter(request, getGrpcMetadata(token), (error: any, response: any) => {
