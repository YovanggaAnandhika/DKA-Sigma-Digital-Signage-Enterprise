...
import { DeleteTransitionRequest } from '@/lib/api/studio/v1/transition/transition.common_pb';
...
export async function POST(req: NextRequest) {
  try {
    ...
    const request = new DeleteTransitionRequest();
    if (body.id) request.setId(body.id);
    
    return new Promise<NextResponse>((resolve) => {
      client.deleteTransition(request, getGrpcMetadata(token), (error: any, response: any) => {
