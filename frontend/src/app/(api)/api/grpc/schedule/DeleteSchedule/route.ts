...
import { DeleteScheduleRequest } from '@/lib/api/generated/studio/v1/schedule/schedule.common_pb';
...
export async function POST(req: NextRequest) {
  try {
    ...
    const request = new DeleteScheduleRequest();
    if(body.id) request.setId(body.id);

    return new Promise((resolve) => {
      client.deleteSchedule(request, getGrpcMetadata(token), (error: any, response: any) => {
