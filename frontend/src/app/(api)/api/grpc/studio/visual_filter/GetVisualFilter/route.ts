...
import { GetVisualFilterRequest } from '@/lib/api/studio/v1/visual_filter/visual_filter.common_pb';
...
export async function POST(req: NextRequest) {
  try {
    ...
    const request = new GetVisualFilterRequest();
    if (body.id) request.setId(body.id);
    
    return new Promise<NextResponse>((resolve) => {
      client.getVisualFilter(request, getGrpcMetadata(token), (error: any, response: any) => {
