...
interface LayoutLayerListProps {
  layout: Layout;
...
export default function LayoutLayerList({
  layout,
  selectedLayerId,
  setSelectedLayerId,
  resolveZoneMedia,
  ...
  playlists,
  mediaList
}: LayoutLayerListProps) {
  const selectedZone = layout.layersList?.find((z) => z.id === selectedLayerId);
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '16px', position: 'sticky', top: '24px', maxHeight: 'calc(100vh - 180px)', overflowY: 'auto' }}>
      ...
      {selectedZone && (() => {
        const color = zColors[(layout.layersList?.findIndex(z => z.id === selectedZone.id) || 0) % zColors.length];
        return (
          <div className="card-elevated" style={{ overflow: 'hidden' }}>
