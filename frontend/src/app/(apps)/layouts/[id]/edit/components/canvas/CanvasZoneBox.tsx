...
interface CanvasZoneBoxProps {
  zone: Zone;
  idx: number;
}

export default function CanvasZoneBox({ zone: z, idx }: CanvasZoneBoxProps) {
  const {
    setZones,
    selectedZoneId,
    setSelectedZoneId,
  ...
  return (
    <Rnd
      ...
      onDrag={(e, d) => {
        ...
        setZones((prev) =>
          prev.map((zone) =>
            zone.id === z.id ? { ...zone, x: nextX, y: nextY } : zone
          )
        );
      }}
      onDragStop={(e, d) => {
      ...
      onResize={(e, direction, ref, delta, position) => {
        ...
        setZones((prev) =>
          prev.map((zone) =>
            zone.id === z.id
              ? { ...zone, width: nextW, height: nextH, x: nextX, y: nextY }
              : zone
          )
        );
      }}
